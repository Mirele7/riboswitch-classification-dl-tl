...
parser.add_argument('--early_stopping', type=int, default=30, help='Number of epochs for early stopping (patience window)')
...
def get_labels_for_indices(indices):
    ...
    for i in indices:
        data = dataset[int(i)]
        ys.append(int(data.y) if hasattr(data, 'y') else int(data.y.item()))
    return np.array(ys)
...
def validate_epoch(model, val_loader):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            pred = out.max(1)[1]
            accuracies.append(compute_metrics_family(data.y, pred))
...
def run(model, optimizer, n_epochs, train_loader, val_loader, fold, results_dir, model_dir):
    ...
    best_val_acc = -1.0
    best_path = os.path.join(model_dir, f'ML_model_set1_fold_{fold}.pt')
    ...
    for epoch in range(n_epochs):
        ...
        if val_accuracy > best_val_acc:
            ...
            print(f"Saved updated model for fold {fold} (val_acc={best_val_acc:.4f})")
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        model, optimizer = make_model_and_optim()  # novo modelo/otimizador por fold
        best_model_path = run(model, optimizer, opt.n_epochs, train_loader, val_loader, fold, results_dir, model_dir)
        best_model_paths.append(best_model_path)

    # ----------------------
    # Selecionar melhor fold para exportar (opcional)
    # ----------------------
    best_model_index = -1
    best_val_acc = -1.0
    for i, path in enumerate(best_model_paths):
        ...
        print(f"Fold {i}: Best val accuracy = {max_val_acc:.4f}")
    ...
    if best_model_index >= 0:
        import shutil
        final_model_path = os.path.join(model_dir, 'ML_set1_best_model.pt')
        shutil.copy(best_model_paths[best_model_index], final_model_path)
        print(f"Copied best fold model to: {final_model_path}")

    # ----------------------
    # (Opcional) Refit em 100% dos dados
    # ----------------------
    if opt.refit_full:
        print("\n[Refit] Training final model on 100% of data (with small internal val split for early stopping)...")
        ...
        final_full_path = os.path.join(model_dir, 'ML_set1_model_full.pt')
        import shutil
        shutil.copy(best_full_path, final_full_path)
        print(f"[Refit] Final model saved at: {final_full_path}")

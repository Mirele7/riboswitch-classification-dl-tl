...
# Data Loading
# ----------------------
n_classes = 2  #two classes (riboswitch and genomic background)
...
def run(model, optimizer, n_epochs, train_loader, val_loader, fold, results_dir, model_dir):
    ...
    best_val_acc = -1.0
    best_path = os.path.join(model_dir, f'DL_model_DISTANCE_fold_{fold}.pt')
    ...
    for epoch in range(n_epochs):
        ...
        plot_loss(train_losses, val_losses, file_name=os.path.join(results_dir, f'DL_DISTANCE_loss_fold_{fold}.jpg'))
        plot_loss(train_accuracies, val_accuracies, file_name=os.path.join(results_dir, f'DL_DISTANCE_acc_fold_{fold}.jpg'), y_label='accuracy')

        with open(os.path.join(results_dir, f'DL_DISTANCE_scores_fold_{fold}.pkl'), 'wb') as f:
            pickle.dump({
...
def main():
    ...
    for i, path in enumerate(best_model_paths):
        scores_path = os.path.join(results_dir, f'DL_DISTANCE_scores_fold_{i}.pkl')
    ...
    if best_model_index >= 0:
        import shutil
        final_model_path = os.path.join(model_dir, 'DL_DISTANCE_best_model.pt')
    ...
    if opt.refit_full:
        ...
        final_full_path = os.path.join(model_dir, 'DL_DISTANCE_model_full.pt')
        import shutil
        shutil.copy(best_full_path, final_full_path)
        print(f"[Refit] Final model saved at: {DL_DISTANCE_model_full}")

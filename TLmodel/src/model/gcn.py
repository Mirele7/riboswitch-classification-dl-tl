...
class TransferGCN(nn.Module):
    def __init__(self, n_features, hidden_dim, n_classes, n_conv_layers=3, dropout=0,
        ...
        if pretrained_modelo_path:
            # Carrega estado do pré-treino sem forçar a cabeça antiga
            state = torch.load(pretrained_modelo_path, map_location=device)
            state.pop('fc.weight', None)
            state.pop('fc.bias', None)

            # Constrói o backbone com a cabeça já no número de classes de destino
            ...
            # A nova 'fc' permanece treinável por padrão

            self.pretrained_model.device = device

        else:
            # Caminho sem pré-treino
            if num_embeddings:
    ...
    def reset_parameters(self):
        """Não zera o backbone pré-treinado; apenas reseta a cabeça de classificação.
        Para o caminho sem pré-treino, reseta tudo normalmente.
        """

...
from src.model.gcn import GCN, TransferGCN
...
parser.add_argument('--foldings_dataset', type=str, default='../data/train_set1.pkl', help='Path to foldings')
parser.add_argument('--train_dataset', type=str, default='../data/train_set1.fa', help='Path to training dataset')
parser.add_argument('--pretrained_modelo_path', type=str, default='../data/pre_train_7_classes_fold_0_bsb_2025.pt', help='Path to the pretrained model with 12 classes')

# NEW FLAGS
parser.add_argument('--k_folds', type=int, default=2, help='Number of CV folds')
...
def make_model_and_optim():
    """Cria um novo modelo + otimizador (para cada fold/refit), garantindo estado limpo."""
    model = TransferGCN(
...
def unfreeze_layers(model, layer_keywords):
    """
    Ativa requires_grad=True para camadas cujos nomes contenham as palavras-chave.
    """
...
def train_epoch(model, optimizer, train_loader):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # acumula loss ponderando pelo tamanho do batch
...
def run(model, optimizer, n_epochs, train_loader, val_loader, fold, results_dir, model_dir):
    ...
    best_val_acc = -1.0
    best_path = os.path.join(model_dir, f'model_100_fold_{fold}.pt')

    for epoch in range(n_epochs):
        start = time.time()
        print(f"\nEpoch {epoch + 1}/{n_epochs}")

        # Descongelamento progressivo de camadas
        if epoch == 1:
        ...
        plot_loss(train_losses, val_losses, file_name=os.path.join(results_dir, f'loss_fold_{fold}.jpg'))
        plot_loss(train_accuracies, val_accuracies, file_name=os.path.join(results_dir, f'acc_fold_{fold}.jpg'), y_label='accuracy')

        with open(os.path.join(results_dir, f'scores_fold_{fold}.pkl'), 'wb') as f:
            pickle.dump({
        ...
        if val_accuracy > best_val_acc:
            ...
            print(f"Modelo salvo para o fold {fold} (val_acc={best_val_acc:.4f})")

        # Early stopping (janela)
...
def get_labels_for_indices(indices):
    """Extrai y para índices do dataset, supondo que cada item tem .y."""
    ys = []
    for i in indices:
        data = dataset[int(i)]
...
def main():
    ...
    with open(os.path.join(results_dir, 'hyperparams.txt'), 'w') as f:
        f.write(str(opt))
    with open(os.path.join(results_dir, 'hyperparams.pkl'), 'wb') as f:
        pickle.dump(opt, f)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        # Modelo e otimizador NOVOS por fold
    ...
    # ----------------------
    # Selecionar melhor fold (como no script original)
    # ----------------------
    best_model_index = -1
    best_val_acc = -1.0

    for i, path in enumerate(best_model_paths):
        scores_path = os.path.join(results_dir, f'scores_fold_{i}.pkl')
        with open(scores_path, 'rb') as f:
            scores = pickle.load(f)
        max_val_acc = max(scores['val_accuracies']) if len(scores['val_accuracies']) > 0 else -1.0
        print(f"Fold {i}: Melhor acurácia de validação = {max_val_acc:.4f}")
        if max_val_acc > best_val_acc:
            best_val_acc = max_val_acc
            best_model_index = i

    best_model_path = best_model_paths[best_model_index]
    print(f"\nMelhor modelo: Fold {best_model_index} com acurácia {best_val_acc:.4f}")
    print(f"Caminho do melhor modelo: {best_model_path}")

    import shutil
    final_model_path = os.path.join(model_dir, 'best_model.pt')
    shutil.copy(best_model_path, final_model_path)
    print(f"Copiado para: {final_model_path}")

    # ----------------------
    # (Opcional) Refit em 100% dos dados
    # ----------------------
    if opt.refit_full:
        print("\n[Refit] Treinando um modelo final em 100% dos dados...")
        # Split interno para early stopping (estratificado)
        ...
        final_full_path = os.path.join(model_dir, 'final_model_full.pt')
        shutil.copy(best_full_path, final_full_path)
        print(f"[Refit] Modelo final salvo em: {final_full_path}")
...
if __name__ == "__main__":
    ...
    print(f"\nTempo total de treino: {(end-start)/60:.2f} minutos")

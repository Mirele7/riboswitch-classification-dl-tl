...
from src.model.gnn import TransferGNN  # <<< TL model
...
#The tests are performed for each riboswitches (NUMBER)
#The model (low, medium, high) must also be informed (DISTANCE)
#The (low, medium, high) hyperparameters must also be informed (DISTANCE)
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_name', default="test", help='model name (folder name)')
    parser.add_argument('--test_dataset',
                        default='../data/test_data/NUMBER_riboswitch/riboswitch_windows_dual.fa',
                        help='Path to test dataset (.fa)')
    parser.add_argument('--foldings_dataset', default='../data/test_data/NUMBER_riboswitch/riboswitch_windows_dual.pkl',
                        help='Path to foldings dataset (.pkl)')
    parser.add_argument('--checkpoint',
                        default=None,
                        help=('Path to model checkpoint'
                              'If not set, uses ../models/test/TL_DISTANCE_model_full.pt'))
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--out_csv', default='../results_model/output_DISTANCE_NUMBER_riboswitch.csv')

    #Additional outputs
    parser.add_argument('--out_txt_all', default=None,
                        help='TXT with all windows: >id_global_group  p_riboswitch=...')
    parser.add_argument('--out_txt_hits_by_group', default=None,
                        help='TXT summary by group listing only predicted riboswitch windows')
    ...
    # --- Load hyperparams ---
    opt_path = f'../results_model/{args.model_name}/DISTANCE_hyperparams.pkl'
    opt = pickle.load(open(opt_path, "rb"))
    opt.device = device  # force consistent device

    # --- Build TL model (TransferGNN) ---
    model = TransferGNN(
    ...
    checkpoint_path = args.checkpoint or f'../models/test/TL_DISTANCE_model_full.pt'
    ...
    with open(out_txt_hits, "w", encoding="utf-8") as f:
        ...
        for gid in unique_groups:
            ...
            f.write(f">Group_{gid} Identified {len(hits)}:\n")
